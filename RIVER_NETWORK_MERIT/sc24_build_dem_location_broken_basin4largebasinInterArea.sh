...
#SBATCH -n 1 -c 1 -N 1
#SBATCH -t 168:00:00
#SBATCH -o /gpfs/scratch60/fas/sbsc/ga254/grace0/stdout/sc24_build_dem_location_broken_basin4largebasinInterArea.sh.%A_%a.out 
#SBATCH -e /gpfs/scratch60/fas/sbsc/ga254/grace0/stderr/sc24_build_dem_location_broken_basin4largebasinInterArea.sh.%A_%a.err
#SBATCH --mail-type=ALL
#SBATCH --mail-user=email
#SBATCH --job-name=sc24_build_dem_location_broken_basin4largebasinInterArea.sh
#SBATCH --array=131-132
...
# 
r.stream.extract elevation=elv  accumulation=upa threshold=0.5    depression=dep     direction=dir  stream_raster=stream memory=45000 --o --verbose  ;  r.colors -r stream
g.remove -f  type=raster name=upa,elv
/gpfs/home/fas/sbsc/ga254/.grass7/addons/bin/r.stream.basins -l  stream_rast=stream  direction=dir  basins=lbasin        memory=45000 --o --verbose  ;  r.colors -r lbasin



r.out.gdal --overwrite -c  createopt="COMPRESS=DEFLATE,ZLEVEL=9" type=UInt32 format=GTiff nodata=0  input=lbasin  output=/gpfs/scratch60/fas/sbsc/ga254/grace0/dataproces/RIVER_NETWORK_MERIT/lbasin_unit_large/lbasin_brokb$UNIT.tif
r.out.gdal --overwrite -c  createopt="COMPRESS=DEFLATE,ZLEVEL=9" type=UInt32 format=GTiff nodata=0  input=stream  output=/gpfs/scratch60/fas/sbsc/ga254/grace0/dataproces/RIVER_NETWORK_MERIT/stream_unit_large/stream_brokb$UNIT.tif
