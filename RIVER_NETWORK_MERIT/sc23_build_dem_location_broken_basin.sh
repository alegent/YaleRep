...
# 132 broken basin  ; the last 2 are excluded 
...
source /gpfs/home/fas/sbsc/ga254/scripts/general/create_location_grass7.0.2-grace2.sh $GRASS loc_$UNIT $RAM/UNIT${UNIT}_elv.tif
...
r.stream.extract elevation=elv  accumulation=upa threshold=0.5    depression=dep     direction=dir  stream_raster=stream memory=35000 --o --verbose  ;  r.colors -r stream
g.remove -f  type=raster name=upa,elv
/gpfs/home/fas/sbsc/ga254/.grass7/addons/bin/r.stream.basins -l  stream_rast=stream  direction=dir  basins=lbasin        memory=35000 --o --verbose  ;  r.colors -r lbasin
