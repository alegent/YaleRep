...
for UNIT in $( awk '{print $1}' /gpfs/scratch60/fas/sbsc/ga254/grace0/dataproces/RIVER_NETWORK/grassdb/occurance_N_DIM.txt | uniq ) ; do 
    grep ^$UNIT  /gpfs/scratch60/fas/sbsc/ga254/grace0/dataproces/RIVER_NETWORK/grassdb/occurance_N_DIM.txt | awk '{ if($2 > 100 ) { print }  }' > /gpfs/scratch60/fas/sbsc/ga254/grace0/dataproces/RIVER_NETWORK/grassdb/occurance_${UNIT}_DIM.txt
done 
...
for file in $( ls /gpfs/scratch60/fas/sbsc/ga254/grace0/dataproces/RIVER_NETWORK/grassdb/occurance_200_DIM.txt ) ; do 
     ...
     PREV_JOB=$(bsub -W 00:01  -n 1  -o /gpfs/scratch60/fas/sbsc/ga254/grace0/stdout/start.out -e /gpfs/scratch60/fas/sbsc/ga254/grace0/stderr/start.err bash /gpfs/home/fas/sbsc/ga254/scripts/general/start_bsub.sh | cut -d'<' -f2 | cut -d'>' -f1 )  
...
for file in $( ls /gpfs/scratch60/fas/sbsc/ga254/grace0/dataproces/RIVER_NETWORK/grassdb/occurance_???_DIM.txt ) ; do 
     echo lunch the start scritp 
     PREV_JOB=$(bsub -W 00:01  -n 1  -o /gpfs/scratch60/fas/sbsc/ga254/grace0/stdout/start.out -e /gpfs/scratch60/fas/sbsc/ga254/grace0/stderr/start.err bash /gpfs/home/fas/sbsc/ga254/scripts/general/start_bsub.sh | cut -d'<' -f2 | cut -d'>' -f1 )  
     for LINE in  $(cat $file | tr " " "_"  )  ; do
         echo lunch the start scritp N $N DIM $DIM  UNIT  $UNIT  
