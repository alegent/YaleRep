#  for SENS in MYD MOD ; do for DN in Day Nig ; do  qsub  -v SENS=$SENS,DN=$DN    /u/gamatull/scripts/LST/spline/sc6_fillsplineAkima_MOYD11A2_allobs.sh ; done ; done 
...
#PBS -V
#PBS -N sc6_fillsplineAkima_MOYD11A2_allobs.sh
#PBS -o /nobackup/gamatull/stdout
#PBS -e /nobackup/gamatull/stderr

echo $SENS /u/gamatull/scripts/LST/spline/sc6_fillsplineAkima_MOYD11A2_allobs.sh 
...
gdalbuildvrt -te $geo_string   -separate   -overwrite  $RAMDIR/LST_${SENS}_${tile}B.vrt \
$( echo $(awk -v nr=$nr -v  INSENS=$INSENS -v SENS=$SENS -v DN=$DN  \'{ if (NR>nr) { print    INSENS "/LST_" SENS "_QC_day" $1 "_wgs84_" DN "_allobs_mask5noobs_Fill5obs_maskoutIndia.tif" } }\' /nobackup/gamatull/dataproces/LST/geo_file/list_day.txt ; awk -v INSENS=$INSENS -v SENS=$SENS -v  DN=$DN   \'{ print INSENS "/LST_" SENS "_QC_day" $1 "_wgs84_" DN "_allobs_mask5noobs_Fill5obs_maskoutIndia.tif" }\' /nobackup/gamatull/dataproces/LST/geo_file/list_day.txt ; awk -v nrt=$nrt -v INSENS=$INSENS -v SENS=$SENS -v DN=$DN \'{ if (NR<=nrt) print  INSENS "/LST_" SENS "_QC_day" $1 "_wgs84_" DN "_allobs_mask5noobs_Fill5obs_maskoutIndia.tif" }\' /nobackup/gamatull/dataproces/LST/geo_file/list_day.txt)) 

gdalinfo  $RAMDIR/LST_${SENS}_${tile}B.vrt 
...
pkfilter -of GTiff -ot Float32 -co BIGTIFF=YES -co COMPRESS=LZW -co ZLEVEL=9 -nodata 0 -f smoothnodata -dz 1 -interp akima -i $RAMDIR/LST_${SENS}_${tile}B.vrt -o $OUTSENS/LST_${SENS}_akima_${DN}_${tile}_tmpB.tif

cat   /proc/meminfo  | grep MemFree


rm -f $RAMDIR/LST_${SENS}_${tile}B.vrt 
gdalbuildvrt      $(seq  $(( 46-$nr+1 )) $(( 46+(46-$nr) )) |  xargs -n 1 echo -b)  $RAMDIR/LST_${SENS}_${tile}B.vrt    $OUTSENS/LST_${SENS}_akima_${DN}_${tile}_tmpB.tif 

gdalinfo $RAMDIR/LST_${SENS}_${tile}B.vrt

gdal_translate  -co COMPRESS=LZW -co ZLEVEL=9 -co BIGTIFF=YES  $RAMDIR/LST_${SENS}_${tile}B.vrt $OUTSENS/LST_${SENS}_akima_${DN}_${tile}_allobsB.tif
rm -f $OUTSENS/LST_${SENS}_akima_${DN}_${tile}_tmpB.tif $RAMDIR/LST_${SENS}_${tile}B.vrt 
...
qsub -v SENS=$SENS,DN=$DN  /u/gamatull/scripts/LST/spline/sc7_fillsplineAkima_merge_tile_MOYD11A2_allobs.sh  
