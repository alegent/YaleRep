#  cd /tmp/ ; for SENS in MYD MOD ; do for DN in Day Nig ; do  qsub  -v SENS=$SENS,DN=$DN  /u/gamatull/scripts/LST/spline/sc9_spline15_IndPat_MOYD11A2.sh  ; done ; done  ; cd -
...
#PBS -N sc9_spline15_IndPat_MOYD11A2.sh

echo  /u/gamatull/scripts/LST/spline/sc9_spline15_IndPat_MOYD11A2.sh
...
rm -f  $RAMDIR/list_day_${SENS}_$tile.txt 
for DAY in  $( cat  /nobackup/gamatull/dataproces/LST/geo_file/list_day.txt )  ; do 

if [ -f $INSENS/LST_${SENS}_akima_${DN}_day${DAY}_allobs_MskIndia.tif ] ; then 
echo    $INSENS/LST_${SENS}_akima_${DN}_day${DAY}_allobs_MskIndia.tif >>    $RAMDIR/list_day_${SENS}_${tile}_tmp.txt 
else 
echo    $INSENS/LST_${SENS}_akima_${DN}_day${DAY}_allobs_Fill5obs.tif >>    $RAMDIR/list_day_${SENS}_${tile}_tmp.txt 
fi 

done 

tail  -$(expr 46 - $nr )  $RAMDIR/list_day_${SENS}_${tile}_tmp.txt  >  $RAMDIR/list_day_${SENS}_${tile}.txt 
cat   $RAMDIR/list_day_${SENS}_${tile}_tmp.txt                      >>  $RAMDIR/list_day_${SENS}_${tile}.txt
head  -$nrt   $RAMDIR/list_day_${SENS}_${tile}_tmp.txt             >>  $RAMDIR/list_day_${SENS}_${tile}.txt 
