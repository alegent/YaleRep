...
# for MMDD in $(cat  /nobackup/gamatull/dataproces/LST/geo_file/mm_dd_MYD11A2.txt ) ; do qsub -v MMDD=$MMDD /u/gamatull/scripts/LST/sc1_local_MYD11A2.sh  ; done 
...
ls    $INDIR/${YEAR}.${MM}.${DD}/${SENS}11A2.A${YEAR}???.??????.???.*.hdf    >  $LST/${SENS}11A2/$YEAR/${SENS}11A2.A${YEAR}${DAY}_hdf.txt 
