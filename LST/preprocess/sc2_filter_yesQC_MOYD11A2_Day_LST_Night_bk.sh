# for DAY in $(cat /nobackup/gamatull/dataproces/LST/geo_file/list_day.txt  ) ; do qsub -v DAY=$DAY,SENS=MOD,DN=$DN /u/gamatull/scripts/LST/preprocess/sc2_mean_yesQC_MOYD11A2_Day_LST_Night.sh ; done
...
echo 29500  8000 2000 1000 f  >> $RAMDIR/tiles_xoff_yoff.txt  # 27000
...
gdalbuildvrt  -overwrite  -b 1  $RAMDIR/LST_${SENS}_QC_day${DAY}_$DN.vrt  $RAMDIR/${SENS}11A2.A????${DAY}_Day_tile${tile}_Day.tif   ${filename}_tile?_$DN.tif   2> /dev/null 

# ~/bin/pkfilter --filter savgolay  -nl 2  -nr 2 -ld 0 -m 2 -i -dx 1 -dy 1 -dz 12 -i $RAMDIR/LST_${SENS}_QC_day${DAY}_$DN.vrt -o $RAMDIR/LST_${SENS}_QC_day${DAY}_${DN}_fsg.tif
