...
# file=$1 
# gdal_translate  --config GDAL_NUM_THREADS 2  --config GDAL_CACHEMAX  10000  -of XYZ   $file  $DIR/global_prediction/$(basename $file .tif ).txt 
...
# file=$1 
# gdal_translate  --config GDAL_NUM_THREADS 2  --config GDAL_CACHEMAX  10000  -of XYZ   $file  $DIR/FLOK1/$(basename $file .tif ).txt
...
# pksetmask -co COMPRESS=DEFLATE -co ZLEVEL=9 -m $DIR/global_prediction/map_pred_NO3_mask.tif -msknodata -1 -nodata -9999 -i $DIR/global_wsheds/global_grid_ID.tif -o  $DIR/global_wsheds/global_grid_ID.tif
# gdal_translate --config GDAL_NUM_THREADS 2 --config GDAL_CACHEMAX 10000 -of XYZ  $DIR/global_wsheds/global_grid_ID.tif  $DIR/global_wsheds/global_grid_ID.txt 
# awk ' { if($3 != -9999 ) print $3  }'  $DIR/global_wsheds/global_grid_ID.txt   > $DIR/global_wsheds/global_grid_ID_clean.txt
# rm -f $DIR/global_wsheds/global_grid_ID.txt 

echo "FID,Cell_ID,Qmax,Qmean,Qmin,S,NH4,NO3,TN,Tp" > $DIR/emision_table.txt

paste <( seq 1 19892976) $DIR/global_wsheds/global_grid_ID_clean.txt  \
...
$DIR/global_prediction/map_pred_DNH4_mask_clean.txt $DIR/global_prediction/map_pred_NO3_mask_clean.txt $DIR/global_prediction/map_pred_TN_mask_clean.txt \
 <(awk ' {print $1/10 }'  $DIR/global_wsheds/tmean_wavg_stream_clean.txt) | awk '{ if ($3 != -1 ) print  }'  >> $DIR/emision_table.txt

cd $DIR 
tar -czvf emision_table.tar.gz emision_table.txt
