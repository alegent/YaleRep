...
file=$(ls $NHD/ds641_nasqan_wbd12_wgs84/*.shp  | tail -n  $SLURM_ARRAY_TASK_ID | head -1 )
...
filename=$( basename  $file  .shp )
ogr2ogr -overwrite -spat $(ogrinfo -al -so $file | grep Extent | awk '{ gsub ("[(),]",""); print $2,$3,$5,$6 }') $NHD/ds641_nasqan_wbd12_wgs84_crop/${filename}_streams.shp $NHD/shp/NHDFlowline.vrt
