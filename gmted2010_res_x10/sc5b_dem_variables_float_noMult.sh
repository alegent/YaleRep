...
###############  VRM
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9  --co=INTERLEAVE=BAND -A $OUTDIR/slope/tiles/${filename}_${mm}.tif --calc="(sin(A * 3.141592 / 180 ))" --outfile   $OUTDIR/slope/tiles/${filename}_${mm}"_sin.tif" --overwrite --type=Float32
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9  --co=INTERLEAVE=BAND -A $OUTDIR/slope/tiles/${filename}_${mm}.tif --calc="(cos(A * 3.141592 / 180))" --outfile   $OUTDIR/slope/tiles/${filename}_${mm}"_cos.tif" --overwrite --type=Float32
####
...
pkfilter -nodata -9999 -dx 3 -dy 3 -f sum  -co COMPRESS=LZW -co ZLEVEL=9 -co INTERLEAVE=BAND -ot Float32   -i   $OUTDIR/slope/tiles/${filename}_${mm}"_cos.tif" -o   $TMP/${filename}_${mm}"_sumz.tif" 

echo x 

gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9 --co=INTERLEAVE=BAND -A  $OUTDIR/slope/tiles/${filename}_${mm}"_sin.tif"   -B  $OUTDIR/aspect/tiles/${filename}_${mm}"_sin.tif"  --calc="(A * B)" --type=Float32 --overwrite --outfile   $TMP/${filename}_${mm}"_x.tif"
...
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9 --co=INTERLEAVE=BAND -A  $OUTDIR/slope/tiles/${filename}_${mm}"_sin.tif"   -B  $OUTDIR/aspect/tiles/${filename}_${mm}"_cos.tif"  --calc="(A * B)" --type=Float32 --overwrite --outfile   $TMP/${filename}_${mm}"_y.tif"
...
gdal_calc.py  --NoDataValue=-9999   --co=COMPRESS=LZW --co=ZLEVEL=9 --co=PREDICTOR=3 --co=INTERLEAVE=BAND -A  $TMP/tmp_${filename}_${mm}.tif --calc="sqrt(A)" --type=Float32  --overwrite --outfile $OUTDIR/vrm/stdev/tiles/${filename}_${mm}.tif
rm -f $TMP/tmp_${filename}_${mm}.tif
