...
pkcreatect   -co COMPRESS=LZW -co ZLEVEL=9   -ct   $RAM/color.txt   -i $OUTGEO/forms/${filename}_tmp.tif  -o  $OUTGEO/forms/${filename}_s3.tif  
...
# perform same operation but with search = 5

/home/fas/sbsc/ga254/.grass7/addons/bin/r.geomorphon   dem=$filename  forms=forms_$filename ternary=ternary_$filename positive=positive_$filename negative=negative_$filename \
              intensity=intensity_$filename exposition=exposition_$filename range=range_$filename variance=variance_$filename \
              elongation=elongation_$filename azimuth=azimuth_$filename extend=extend_$filename width=with_$filename \
              search=5 skip=0 flat=1 dist=0 step=0 start=0 --overwrite

r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Byte     input=forms_$filename         output=$OUTGEO/forms/${filename}_tmp.tif 
pkcreatect  -min 0 -max 10  > $RAM/color.txt
pkcreatect   -co COMPRESS=LZW -co ZLEVEL=9   -ct   $RAM/color.txt   -i $OUTGEO/forms/${filename}_tmp.tif  -o  $OUTGEO/forms/${filename}_s5.tif  
rm $OUTGEO/forms/${filename}_tmp.tif 

r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Int16    input=ternary_$filename       output=$OUTGEO/ternary/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Byte     input=positive_$filename      output=$OUTGEO/positive/${filename}_s5.tif  
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Byte     input=negative_$filename      output=$OUTGEO/negative/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Float32  input=intensity_$filename     output=$OUTGEO/intensity/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Int16    input=exposition_$filename    output=$OUTGEO/exposition/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=UInt16   input=range_$filename         output=$OUTGEO/range/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Float32  input=variance_$filename      output=$OUTGEO/variance/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Float32  input=elongation_$filename    output=$OUTGEO/elongation/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Float32  input=azimuth_$filename       output=$OUTGEO/azimuth/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Float32  input=extend_$filename        output=$OUTGEO/extend/${filename}_s5.tif 
r.out.gdal -c     createopt="COMPRESS=LZW,ZLEVEL=9" format=GTiff  type=Float32  input=with_$filename          output=$OUTGEO/with/${filename}_s5.tif  
