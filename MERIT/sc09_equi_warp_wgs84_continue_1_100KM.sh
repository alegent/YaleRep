...
echo starting the merging $TOPO 

for PAR in altit stdev ; do 
export PAR
echo  AF AN AS EU NA OC SA | xargs -n 1 -P 6  bash -c $'
CT=$1

gdalwarp -overwrite  -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -r bilinear -srcnodata -9999 -dstnodata -9999 -tr ${RES:0:22} ${RES:0:22} -te -180.0000000 -60.0000000 180.0000000 85.0000000 -s_srs  $EQUI/${CT}/PROJ/EQUI7_V13_${CT}_PROJ_ZONE.prj    -t_srs $EQUI/${CT}/GEOG/EQUI7_V13_${CT}_GEOG_ZONE.prj $SCRATCH/$TOPO/$MATH/${CT}_tiles_km${KM}_${PAR}.tif $RAM/${TOPO}_${MATH}_${CT}_tiles_km${KM}_${PAR}.tif 

pksetmask -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -m $EQUI/$CT/GEOG/EQUI7_V13_${CT}_GEOG_ZONE_KM$KM.tif  -msknodata 0 -nodata -9999 -i $RAM/${TOPO}_${MATH}_${CT}_tiles_km${KM}_${PAR}.tif -o  $SCRATCH/$TOPO/$MATH/${CT}_tiles_km${KM}_${PAR}_wgs84.tif 

rm $RAM/${TOPO}_${MATH}_${CT}_tiles_km${KM}_${PAR}.tif

' _

if [ $PAR == "altit"  ] ; then  PARN=altitude ; else PARN=stdev ; fi  
echo start pkstatprofile 
gdalbuildvrt  -overwrite  -separate  $SCRATCH/$TOPO/$MATH/GLOBE_tiles_km${KM}_${PAR}_wgs84.vrt   $SCRATCH/$TOPO/$MATH/??_tiles_km${KM}_${PAR}_wgs84.tif
pkstatprofile -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -nodata -9999 -f mean  -i  $SCRATCH/$TOPO/$MATH/GLOBE_tiles_km${KM}_${PAR}_wgs84.vrt -o  $MERIT/$TOPO/$MATH/${PARN}_${KM}KM${MATH}_MERIT_E7.tif 
rm   $SCRATCH/$TOPO/$MATH/GLOBE_tiles_km${KM}_${PAR}_wgs84.vrt  $SCRATCH/$TOPO/$MATH/??_tiles_km${KM}_${PAR}_wgs84.tif
done 
...
pkstatprofile -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -nodata -9999 -f mean  -i  $SCRATCH/$TOPO/$MATH/GLOBE_tiles_km${KM}_wgs84.vrt -o  $MERIT/$TOPO/$MATH/${TOPO}_${KM}KM${MATH}_MERIT_E7.tif 
rm   $SCRATCH/$TOPO/$MATH/GLOBE_tiles_km${KM}_wgs84.vrt  $SCRATCH/$TOPO/$MATH/??_tiles_km${KM}_wgs84.tif
...
pkstatprofile -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -nodata -9999 -f mean  -i  $SCRATCH/$TOPO/$MATH/GLOBE_tiles_km${KM}_${FUN}_wgs84.vrt -o  $MERIT/$TOPO/$MATH/${TOPON}_${KM}KM${MATH}_MERIT_E7.tif 
rm   $SCRATCH/$TOPO/$MATH/GLOBE_tiles_km${KM}_${FUN}_wgs84.vrt  $SCRATCH/$TOPO/$MATH/??_tiles_km${KM}_${FUN}_wgs84.tif
done 
fi 
