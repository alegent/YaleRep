...
# Advanced usage: bash install_pktools.sh [--enable-las] [--enable-fann]
#
...
if [ "${FANN}" -eq 1 ] && [ -z "${LAS}"  ]  ;then
cmake -DBUILD_WITH_FANN=ON .. 
fi

if [ "${LAS}" -eq 1 ] && [ -z "${FANN}"  ]  ;then
cmake -DBUILD_WITH_LIBLAS=ON .. 
fi

if [ "${LAS}" -eq 1 ] && [ "${FANN}" -eq 1  ]  ;then
cmake  -DBUILD_WITH_LIBLAS=ON -DBUILD_WITH_FANN=ON .. 
fi

if [ -z "${LAS}" ] && [ -z "${FANN}"  ]  ;then
cmake .. 
fi
...
apt-get -y install libxext6:i386 lsb-core lib32gcc-4.7-dev 
cd /tmp
...
apt-get  -y  install gedit emacs24
