...
MONTH=$1
r.external   input=$RAM/CHELSA_${VAR}_1_${YEAR}_V1.2.tif output=${VAR}_${MONTH}_${YEAR}  --overwrite 
r.mapcalc " ${VAR}_${MONTH}_${YEAR}f = float(${VAR}_${MONTH}_${YEAR}) "

' _
done 

/gpfs/home/fas/sbsc/ga254/.grass7/addons/scripts/r.bioclim tmin=$(g.list type=rast pat=tmin_*f map=. sep=,) \
                                                           tmax=$(g.list type=rast pat=tmax_*f map=. sep=,) \
