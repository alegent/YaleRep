...
# for DAY in $(tail -360 /lustre0/scratch/ga254/dem_bj/AE_C51_MOD04_L2_MYD04_L2/day_list.txt   )  ; do for YEAR in `seq 2002 2002` ; do qsub -v YEAR=$YEAR,DAY=$DAY  /lustre0/scratch/ga254/scripts_bj/environmental-layers/terrain/procedures/dem_variables/AE_C6_MYD04_L2/sc1_heg_tif.sh ; sleep 60 ; done ; done  & 

# bash  /lustre0/scratch/ga254/scripts_bj/environmental-layers/terrain/procedures/dem_variables/AE_C6_MYD04_L2/sc1_heg_tif.sh  2003 100
...
gdalbuildvrt -srcnodata -9999 -vrtnodata -9999 -overwrite  -hidenodata   $TIFDIR/year${YEAR}_day$DAY.vrt   $TIFDIR_TMP/MYD04_L2.A${YEAR}${DAY}*$band.tif
gdal_translate  -a_nodata -9999   -co  COMPRESS=LZW -co ZLEVEL=9         $TIFDIR/year${YEAR}_day$DAY.vrt   $TIFDIR/${band}_year${YEAR}_day$DAY.tif
