...
#PBS -S /bin/bash
#PBS -q fas_normal
#PBS -l walltime=00:10:00  # 1 hour for the full cicle 
#PBS -l nodes=1:ppn=1
#PBS -V
...
gdal_translate -ot Int16     -co  COMPRESS=LZW -co ZLEVEL=9    $OUTDIR/AOD_day$DAY.tif  $OUTDIR/tmpAOD_day$DAY.tif
mv  $OUTDIR/tmpAOD_day$DAY.tif  $OUTDIR/AOD_day$DAY.tif

# rm -f  $OUTDIR/merge$DAY.tif   $OUTDIR/merge$DAY.vrt
