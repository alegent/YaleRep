...
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9  --co=INTERLEAVE=BAND -A $OUTDIR/aspect/tiles/${filename}.tif --calc="(sin(A * 3.141592 / 180 ))" --outfile   $OUTDIR/aspect/tiles/${filename}_sin.tif --overwrite --type=Float32
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9  --co=INTERLEAVE=BAND -A $OUTDIR/aspect/tiles/${filename}.tif --calc="(cos(A * 3.141592 / 180))" --outfile   $OUTDIR/aspect/tiles/${filename}_cos.tif --overwrite --type=Float32
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9  --co=INTERLEAVE=BAND -A $OUTDIR/slope/tiles/${filename}.tif --calc="(sin(A * 3.141592 / 180 ))" --outfile   $OUTDIR/slope/tiles/${filename}_sin.tif --overwrite --type=Float32
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9  --co=INTERLEAVE=BAND -A $OUTDIR/slope/tiles/${filename}.tif --calc="(cos(A * 3.141592 / 180))" --outfile   $OUTDIR/slope/tiles/${filename}_cos.tif --overwrite --type=Float32

echo   Ew  Nw   median 

gdal_calc.py --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9 --co=INTERLEAVE=BAND -A $OUTDIR/slope/tiles/${filename}.tif -B $OUTDIR/aspect/tiles/${filename}_sin.tif --calc="((sin(A))*B)" --outfile  $OUTDIR/aspect/tiles/${filename}_Ew.tif --overwrite --type=Float32
gdal_calc.py --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9 --co=INTERLEAVE=BAND -A $OUTDIR/slope/tiles/${filename}.tif -B $OUTDIR/aspect/tiles/${filename}_cos.tif --calc="((sin(A))*B)" --outfile  $OUTDIR/aspect/tiles/${filename}_Nw.tif --overwrite --type=Float32
...
echo z 
pkreclass  -co COMPRESS=LZW -co ZLEVEL=9 -co INTERLEAVE=BAND -ot Float32  -c -9999  -r 1  -i $OUTDIR/slope/tiles/${filename}_cos.tif     -o $OUTDIR/slope/tiles/${filename}_cos_1.tif

pkfilter -nodata -9999 -dx 3 -dy 3 -f sum  -co COMPRESS=LZW -co ZLEVEL=9 -co INTERLEAVE=BAND  -ot Float32   -i   $OUTDIR/slope/tiles/${filename}_cos_1.tif -o   $RAM/${filename}_sumz1.tif

echo x 

pkreclass  -co COMPRESS=LZW -co ZLEVEL=9  -co INTERLEAVE=BAND  -ot Float32  -c -9999  -r 0  -i $OUTDIR/slope/tiles/${filename}_sin.tif     -o $OUTDIR/slope/tiles/${filename}_sin_0.tif
...
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9 --co=INTERLEAVE=BAND -A  $OUTDIR/slope/tiles/${filename}_sin_0.tif   -B  $OUTDIR/aspect/tiles/${filename}_sin_0.tif  --calc="(A * B)" --type=Float32 --overwrite --outfile   $RAM/${filename}_x.tif
...
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9 --co=INTERLEAVE=BAND -A  $OUTDIR/slope/tiles/${filename}_sin_0.tif   -B  $OUTDIR/aspect/tiles/${filename}_cos.tif  --calc="(A * B)" --type=Float32 --overwrite --outfile   $RAM/${filename}_y.tif
...
gdal_calc.py  --NoDataValue=-9999 --co=COMPRESS=LZW --co=ZLEVEL=9 --co=INTERLEAVE=BAND -A $RAM/${filename}_sumx0.tif  -B  $RAM/${filename}_sumy0.tif  -C  $RAM/${filename}_sumz1.tif  --calc="( 1 - ( (sqrt ( ( A * A ) + (B * B ) + ( C * C ) )) / 8 ) )" --type=Float32 --overwrite --outfile   $RAM/${filename}.tif 

rm -f  $RAM/${filename}_sumx0.tif   $RAM/${filename}_sumy0.tif   $RAM/${filename}_sumz1.tif $RAM/${filename}_*.tif
cp  $RAM/${filename}.tif  $OUTDIR/vrm/tiles/${filename}.tif
