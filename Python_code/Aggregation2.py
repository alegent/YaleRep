...
#                             [-f output_format] [-no noDataValue] [-qa] input_image
#
# Arguments:   -m   metrics to calculate
#                       available metrics: mean, std, max, min, cv, range, skew,
#                                       pct5 (5th percentile), pct50, pct95,
#                                       all (default)
...
def calcStats(Array, statList, NoData=-9999):
...
def Usage():
    ...
    print('                      [-f output_format] [-no noDataValue] [-qa] input_image')
    print('                      [--help-general]')
    print('')
...
# set default values
metric = ['mean','std','max','min','cv','range','skew','qt0.05','qt0.5','qt0.95','count']
outPath = None
refX = None
refY = None
winX = 3
...
if mode == 1:
    for R in xrange(NY):
        ...
        for C in xrange(NX):
            ...
            if win.count() != 0:
                valueDic = calcStats(win, metric, noData)  # call the function
                for m in metric:
