...
t = time.time()
# import LC products
#g = gdal.Open('D:\\Work\\Consensus\\consensus_data\\GlobCover_Oregon_reclass.tif')
...
#                    print 'MODIS is class %d and GlobCover is class %d for pixel (%d, %d)' % (newMODIS[r,c], int(area.keys()[area.values().index(max(area.values()))]), r, c)
                    for lc in range(13):
                        if lc == int(newMODIS[r,c]) or area[str(lc)] > 0:
                            ...
                            con500m[str(lc)][r,c] = round(wpp*100)
                        else:
...
#    output = driver.Create('D:\\Work\\Consensus\\consensus_data\\Oregon_500m_unst_class_%s_v2.tif' % lc, con500m[lc].shape[1], con500m[lc].shape[0], 1, gdal.GDT_Byte)
    output = driver.Create("/home/maoningt/LC/Output/consensus_500m_unst_class_%s.tif" % lc, con500m[lc].shape[1], con500m[lc].shape[0], 1, gdal.GDT_Byte)
