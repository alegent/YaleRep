...
g = gdal.Open('/home/maoningt/LC/Data/nlcd1992_reclass_LL.tif')
NLCD = g.ReadAsArray()
...
for r in range(shape[0]):
    ...
    for c in range(shape[1]):
        ...
        if NLCD.shape[1] > maxCol > 0 and NLCD.shape[0] > maxRow > 0:
            ...
            area = {str(lc):0.0 for lc in range(1,13)}
...
#    output = driver.Create('D:\\Work\\Consensus\\consensus_data\\Oregon_500m_unst_class_%s_v2.tif' % lc, con500m[lc].shape[1], con500m[lc].shape[0], 1, gdal.GDT_Byte)
    output = driver.Create("/home/maoningt/LC/Output/nlcd1992_1km_class_%s.tif" % lc, shape[1], shape[0], 1, gdal.GDT_Byte, ['COMPRESS=LZW'])
