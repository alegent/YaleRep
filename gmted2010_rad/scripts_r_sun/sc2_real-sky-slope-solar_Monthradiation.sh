...
export OUTDIR=months_gr_slope1
...
r.horizon  elevin=$file  horizonstep=15  horizon=horiz
...
# r.sun -s  elevin=$file slope=0  linkein=linke${day}_${filename}_coef    albedo=albedo${day}_${filename}_coef    coefbh=cloud${day}_${filename}_coef     day=$day step=1 \
# glob_rad=glob_HradC_day$day \
# diff_rad=diff_HradC_day$day \
# beam_rad=beam_HradC_day$day \
# refl_rad=refl_HradC_day$day  --overwrite

r.sun -s  elevin=$file  aspin=aspect_$filename  slopein=slope_$filename  linkein=linke${day}_${filename}_coef    albedo=albedo${day}_${filename}_coef    coefbh=cloud${day}_${filename}_coef     day=$day step=1 horizon=horiz  horizonstep=15   --overwrite  \
glob_rad=glob_HradC_day$day \
diff_rad=diff_HradC_day$day \
beam_rad=beam_HradC_day$day \
refl_rad=refl_HradC_day$day  # orizontal 0 reflectance 

# non fatto qui r.mapcalc per che se non si poteva fare il multicore
...
# r.serie puo essere un alternativa
