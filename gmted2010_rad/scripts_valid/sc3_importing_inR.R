...
# insert the cloud in the model to check if there is an impruvemnte 
lm.realOBS.clearMod.cloud  = lm( radiation$globRobs ~ radiation$CLOUD + radiation$clearModel) 
summary(lm.realOBS.clearMod.cloud)

plot(lm.realOBS.clearMod.cloud$fitted.values , radiation$globRobs )
...
xyplot ( globRobs~glob.horiz1 |as.factor(month) , data=real.skyMod.horiz1.slop1.real.skyObs )


xyplot(CLOUD~OBSRAD|as.factor(month),data=d2)

subset ( radiation.cloud , radiation.cloud$CODE == 690150 )

clear=subset ( radiation.cloud , radiation.cloud$CODE == 690150 )[4:15]
cloud=subset ( radiation.cloud , radiation.cloud$CODE == 690150 )[4:15]
