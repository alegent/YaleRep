...
seq 001 017 033 049 065 080 097 113 129 145 161 177 193 209 225 241 257 273 289 305 321 337 353  | xargs -n 1  -P 12  bash  -c $'
day=$(expr $(($day ))) # to have the integer value 

echo  import albedo 
r.external  -o input=/lustre/scratch/client/fas/sbsc/ga254/dataproces/MODALB/0.3_5.0.um.00-04.WS.c004.v2.0/AlbMap.WS.c004.v2.0.00-04.${day}.0.3_5.0.tif   output=albedo${day}_${filename} --overwrite 
r.mapcalc  " albedo${day}_${filename}_coef =  albedo${day}_${filename}  * 0.001" 

echo  import cloud

r.external -o input=/lustre/scratch/client/fas/sbsc/ga254/dataproces/CLOUD/month/MCD09_mean_07.tif   output=cloud${day}_${filename}  --overwrite 
...
r.sun -s  elev_in=$filename  slope=0.01 \
