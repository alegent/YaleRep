# qsub -v DAYc=321,WIND=15 /lustre0/scratch/ga254/scripts_bj/environmental-layers/terrain/procedures/dem_variables/AE_C51_MOD04_L2_MYD04_L2/sc2_window_mean_C51.sh 

# for DAYc  in `tail -360 /lustre0/scratch/ga254/dem_bj/AE_C6_MYD04_L2/day_list.txt`  ; do qsub -v DAYc=$DAYc,WIND=15 /lustre0/scratch/ga254/scripts_bj/environmental-layers/terrain/procedures/dem_variables/AE_C51_MOD04_L2_MYD04_L2/sc2_window_mean_C51_w15.sh  ; done
...
echo processing day $DAYc to create $OUTDIR/AOD_550_mean_day${DAYc}.tif
...
pkcomposite  $(cat $OUTDIR/file4vrt_day${DAYc}.txt               |  xargs -n 1 echo -i $1 )   -min -1 -max 5001 -cr mean  -dstnodata -9999  -srcnodata -9999  -ot Int32   -co COMPRESS=LZW -co ZLEVEL=9   -o $OUTDIR/AOD_550_mean_day${DAYc}.tif
...
# che check min and max in the resulst 
 for n  in `cat /lustre0/scratch/ga254/dem_bj/AE_C6_MYD04_L2/day_list.txt`  ; do pkinfo -src_max 0 -hist  -i  AOD_550_mean_day$n.tif  | head ; echo $AOD_550_mean_day$n.tif ; done  
